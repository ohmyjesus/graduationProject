function [sys,x0,str,ts] = SixJointSMController(t,x,u,flag)
% 以下程序是 神经网络结合常规滑模
switch flag
  case 0 %初始化
...
global c b node gama1 gama2 gama3 gama4 gama5 gama6  c1 c2 c3 c4 c5 c6 xite1 xite2 xite3 xite4 xite5 xite6 k1 k2 k3 k4 k5 k6
% 神经网络采用2-7-1结构
node = 7;              % 高斯函数的中心点矢量 维度 IN * MID  2*7
c = 1 * [-2 -1.5 -1 0 1 1.5 2;
          -2 -1.5 -1 0 1 1.5 2;
          -2 -1.5 -1 0 1 1.5 2;
          -2 -1.5 -1 0 1 1.5 2;
          -2 -1.5 -1 0 1 1.5 2;
          ...
          -2 -1.5 -1 0 1 1.5 2;
          -2 -1.5 -1 0 1 1.5 2];  
b = 10;  % 高斯函数的基宽  维度MID * 1   b的选择很重要 b越大 网路对输入的映射能力越大  
gama1 = 20   ;gama2 = 20;   gama3 = 20;  gama4 = 20;  gama5 = 20;  gama6 = 20;    % 学习率 越大学习越快，但可能导致过拟合
c1 = 15;c2 = 15;c3 = 15;c4 = 15;c5 = 15;c6 = 15;
% 采用指数趋近律设计控制器  为了保证快的趋近速度同时保证小的抖振 应增大k减小xite
xite1 = 0.1;  xite2 = 0.1; xite3 = 0.1; xite4 = 0.1 ;xite5 = 0.1;xite6 = 0.1;
k1 = 80; k2 = 80; k3 = 80; k4 = 80; k5 = 80; k6 = 80; 
sizes = simsizes;
...
for i =1:node
    hf1(i) = exp(-(norm(Input - c(:,i))^2) / (b^2));
    hf2(i) = exp(-(norm(Input - c(:,i))^2) / (b^2));
    hf3(i) = exp(-(norm(Input - c(:,i))^2) / (b^2));
    hf4(i) = exp(-(norm(Input - c(:,i))^2) / (b^2));
    hf5(i) = exp(-(norm(Input - c(:,i))^2) / (b^2));
    hf6(i) = exp(-(norm(Input - c(:,i))^2) / (b^2));
end
dw_fx1 = -gama1 * s1 * hf1; % 7*1矩阵
dw_fx2 = -gama2 * s2 * hf2; % 7*1矩阵
dw_fx3 = -gama3 * s3 * hf3; % 7*1矩阵
dw_fx4 = -gama4 * s4 * hf4; % 7*1矩阵
dw_fx5 = -gama5 * s5 * hf5; % 7*1矩阵
dw_fx6 = -gama6 * s6 * hf6; % 7*1矩阵
...
% 采用指数趋近律设计控制器  为了保证快的趋近速度同时保证小的抖振 应增大k减小xite
temp1 = c1*dqd1 - c1*dq1 + ddqd1 - fx1 - H(1) + xite1*sign(s1) + k1*s1;
temp2 = c2*dqd2 - c2*dq2 + ddqd2 - fx2 - H(2) + xite2*sign(s2) + k2*s2;
temp3 = c3*dqd3 - c3*dq3 + ddqd3 - fx3 - H(3) + xite3*sign(s3) + k3*s3;
temp4 = c4*dqd4 - c4*dq4 + ddqd4 - fx4 - H(4) + xite4*sign(s4) + k4*s4;
temp5 = c5*dqd5 - c5*dq5 + ddqd5 - fx5 - H(5) + xite5*sign(s5) + k5*s5;
temp6 = c6*dqd6 - c6*dq6 + ddqd6 - fx6 - H(6) + xite6*sign(s6) + k6*s6;
